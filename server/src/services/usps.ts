...
class USPSService {
  ...
  /**
   * Validate USPS configuration and return detailed status
   */
  validateConfiguration(): {
    ...
    if (!this.clientId) {
      issues.push('USPS_CLIENT_ID environment variable is not set');
    } else if (this.clientId.length < 10) {
      issues.push('USPS_CLIENT_ID appears to be too short (should be at least 10 characters)');
    }

    if (!this.clientSecret) {
      issues.push('USPS_CLIENT_SECRET environment variable is not set');
    } else if (this.clientSecret.length < 10) {
      issues.push('USPS_CLIENT_SECRET appears to be too short (should be at least 10 characters)');
    }

    if (!this.baseUrl) {
      issues.push('USPS_BASE_URL environment variable is not set');
    } else if (!this.baseUrl.startsWith('https://')) {
      issues.push('USPS_BASE_URL should use HTTPS');
    }
  ...
  async validateAddress(address: {
    ...
    state: string;
    zip: string;
  }): Promise<USPSAddressResponse | null> {
    ...
    try {
      ...
      if (!token) {
        console.warn('[USPS] Cannot validate address - failed to obtain access token');
        return null;
      }
...
export default new USPSService();
