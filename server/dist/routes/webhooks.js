...
router.post('/stripe', express_1.default.raw({ type: 'application/json' }), async (req, res) => {
    try {
        ...
        const event = await stripe_1.default.handleWebhook(req.body, sig);
        ...
        if (event.type === 'checkout.session.completed') {
            ...
            if (session.metadata?.referralCode && session.metadata?.userId) {
                try {
                    const conversionResult = await (0, affiliate_tracking_1.processAffiliateConversion)(session.metadata.referralCode, {
 ...
 */
router.post('/shipstation', express_1.default.text({ type: 'application/xml' }), async (req, res) => {
    try {
