...
router.get('/:productSku/reviews', async (req, res) => {
    try {
        ...
        const formattedReviews = reviews.map(review => ({
            ...
            userName: review.user.firstName ? `${review.user.firstName} ${review.user.lastName?.charAt(0) || ''}.` : 'Anonymous',
        ...
        const reviewStats = await prisma_1.prisma.review.aggregate({
            where: { productId: product.id },
            _avg: { rating: true },
            _count: { id: true }
        });
 ...
 */
router.post('/:productSku/reviews', auth_1.authenticate, async (req, res) => {
    try {
        if (!req.user?.id) {
